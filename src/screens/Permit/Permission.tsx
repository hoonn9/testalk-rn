...
const People: React.FunctionComponent<IProp> = ({navigation}) => {
  ...
  const [setNotifyId] = useMutation<SetUserNotify, SetUserNotifyVariables>(
    SET_USER_NOTIFY,
  );
  ...
  const getPermission = () => {
    ...
    new Promise(async (resolve, reject) => {
      const notifyStatus = await requestNotifications(['alert', 'sound']).then(
        ({status, settings}) => {
          return status;
        },
      );
  ...
  return (
    <Container>
      ...
      <View>
        <Text>
          {'거부하셨던 분들은 설정 - 앱에서 수동으로 허용 해주셔야 합니다.'}
        </Text>
